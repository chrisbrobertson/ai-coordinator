...
function formatExecutionMetrics(durationMs, tokenUsage) {
...
async function runValidationPass(input) {
    ...
    for (const tool of input.roleAssignment.validators) {
        ...
        if (result.exitCode !== 0 && input.logger) {
            input.logger.warn({ tool, exitCode: result.exitCode, durationMs: result.durationMs, tokenUsage: result.tokenUsage }, 'Validator execution returned non-zero exit code');
        ...
        try {
            ...
            if (result.exitCode !== 0 && input.logger) {
                input.logger.warn({ tool, exitCode: result.exitCode, durationMs: result.durationMs, tokenUsage: result.tokenUsage }, 'Validator retry execution returned non-zero exit code');
                if (result.output && result.output.trim().length > 0) {
